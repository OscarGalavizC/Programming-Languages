...
(define (parse-list x)
  (unless (= (length x) 2)
    (error 'parse "expresión no es valida: ~e" x))
  (list-exp (map parse-expression (second x))))

(define (parse-empty-list x)
  (unless (= (length x) 2)
    (error 'parse "expresión no es valida: ~e" x))
  (list-exp null))
...
(define (apply-procedure proc val store)
  ...
  (cond
    [(const-exp? exp)
     (let ([n (const-exp-num exp)])
       (eval (num-val n) store))]
    [(var-exp? exp)
     (let ([var (var-exp-var exp)])
       (eval (apply-env env var) store))]
    [(diff-exp? exp)
     (let ([exp1 (diff-exp-exp1 exp)]
           [exp2 (diff-exp-exp2 exp)])
       (let* ([val1 (value-of exp1 env store)]
              [val2 (value-of exp2 env (eval-store val1))])
         (eval (num-val (- (expval->num (eval-value val1))
                           (expval->num (eval-value val2))))
               (eval-store val2))))]
    [(zero?-exp? exp)
     (let ([exp1 (zero?-exp-exp1 exp)])
       (let ([val1 (value-of exp1 env store)])
         (if (equal? 0 (expval->num (eval-value val1)))
             (eval (bool-val #t) store)
             (eval (bool-val #f) store))))]
    [(if-exp? exp)
     (let ([exp1 (if-exp-exp1 exp)]
       ...
       (let* ([val1 (value-of exp1 env store)]
              [val2 (value-of exp2 env (eval-store val1))]
              [val3 (value-of exp3 env (eval-store val1))])
         (if (expval->bool (eval-value val1))
             (eval (eval-value val2) (eval-store val2))
             (eval (eval-value val3) (eval-store val3)))))]
    [(let-exp? exp)
     (let ([var (let-exp-var exp)]
           [exp1 (let-exp-exp1 exp)]
           [body (let-exp-body exp)])
       (let ([val1 (value-of exp1 env store)])
         (let ([val2 (value-of body
                               (extend-env var
                                           (eval-value val1)
                                           env)
                               (eval-store val1))])
    ...
    [(proc-exp? exp)
     (let ([var (proc-exp-var exp)]
       ...
       (eval (proc-val (procedure var body env)) store))]
    [(call-exp? exp)
     (let ([rator (call-exp-rator exp)]
           [rand (call-exp-rand exp)])
       (let* ([val1 (value-of rator env store)]
             [val2 (value-of rand env (eval-store val1))])
         (let ([proc (expval->proc (eval-value val1))]
               [arg (eval-value val2)])
           (apply-procedure proc arg (eval-store val2)))))]
    [(begin-exp? exp)
    ...
    [(newref-exp? exp)
     (let ([exp1 (newref-exp-cont exp)])
       (let ([cont (value-of exp1 env store)])
         (eval (ref-val (newref (eval-value cont)))
               (list store (eval-value cont)))))]
    [(deref-exp? exp)
     (let ([exp1 (deref-exp-ref exp)])
       (let ([val1 (value-of exp1 env store)])
         (let ([ref (expval->ref (eval-value val1))])
           (eval (deref ref) (eval-store val1)))))]
    [(setref-exp? exp)
     (let ([exp1 (setref-exp-ref exp)]
           [exp2 (setref-exp-cont exp)])
       (let ([val (value-of exp1 env store)])
         (let ([ref (expval->ref (eval-value val))]
               [cont (value-of exp2 env (eval-store val))])
           (begin
             (setref! ref cont)
             (eval (eval-value val) (eval-store cont))))))]
    [else
